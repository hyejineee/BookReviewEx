...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding.searchEditTextView.handleIconClick = handleSearchButton
...
//        }
        ...
        booksViewModel.book.observe(this){
            Log.d(TAG, "state : ${it}")
            when(it){
                BookListState.Idle -> attachListView()
                BookListState.Loading -> handleLoadingState()
                is BookListState.Success -> handleSuccessState(it)
            }
        }
    ...
    private fun handleSuccessState(state:BookListState.Success){
//        adapter.submitList(state.reviewList.toMutableList())

        adapter.submitList(state.books.toMutableList())
    }



    private val handleSearchButton:()->Unit = {
        val keyword = binding.searchEditTextView.text.toString()
        //todo 도서 검색 api로 요청하기
        booksViewModel.searchBook(keyword)
    }
